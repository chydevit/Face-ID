...
final class CameraManager: NSObject {
    ...
    private let sessionQueue = DispatchQueue(label: "com.chydevit.faceid.camera.session")
        ...
        guard permission == .granted else {
            errorMessage = "Camera access not granted (status: \(describe(status))). " +
                (status == .restricted
                    ...
                    : "Enable it in System Settings > Privacy & Security > Camera. If Face ID isn't listed there, quit the app, run `tccutil reset Camera com.chydevit.faceid` in Terminal, then relaunch so macOS asks again.")
            return
        }
